...
author: "John Ormerod and Ellis Patrick"
...
Let's plot a scatterplot of y=weight vs x= height
...
# Generate a heteroscedastic error: the variability of the error increases with x
errors <-  runif(100) * 100

# Create a linear relationship between x and y with the heteroscedastic error
y <- 2 * x + 10 + errors

ggplot(data = NULL, aes(x,y)) + geom_point() + ggtitle("Homoscedasticity")
```
...
# Generate a heteroscedastic error: the variability of the error increases with x
errors <- x * runif(100) * 2

# Create a linear relationship between x and y with the heteroscedastic error
y <- 2 * x + 10 + errors

ggplot(data = NULL, aes(x,y)) + geom_point() + ggtitle("Heteroscedasticity")
```
]
...
and $\beta_1$ using least squares is "best" (Gauss-Markov theorem).
...
.pull-left[
```{r, fig.height=8, warning=FALSE, eval=FALSE}
...
library(ggplot2)
rho <- 0.7
data.frame(x=factor(0:1), prob=dbinom(0:1, 1, rho)) |>
ggplot(aes(x, prob)) + 
...
+ $\widehat{\beta}_0 = `r signif(res5$coef[1],2)`$ and

+ $\widehat{\beta}_1 = `r signif(res5$coef[2],2)`$.
...
g <- ggplot(df, aes(x= temp,y=TD)) +
  geom_point() +
  geom_line(aes(x= temp,y=fit), linewidth=2,col="blue") +
...
$$\beta_i = \text{log(odds(}p|X=x_0 + 1)) - \text{log(odds(}p|X = x_0))$$

and so
$$e^{\beta_i} = \frac{\text{odds}(p|X=x_0 + 1)}{\text{odds(}p|X = x_0)}$$


With our coefficients as $\widehat{\beta}_0 = `r signif(res5$coef[1],2)`$ and $\widehat{\beta}_1 = `r signif(res5$coef[2],2)`$. We have $e^{\widehat{\beta}_0} = `r signif(exp(res5$coef[1]),2)`$ and $e^{\widehat{\beta}_1} = `r signif(exp(res5$coef[2]),2)`$
